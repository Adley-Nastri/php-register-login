...
function activate($email, $c_code){
    $email = mysql_real_escape_string($email);
    $c_code = mysql_real_escape_string($c_code);
    
    if (mysql_result(mysql_query("SELECT COUNT(`UserID`) FROM `users` WHERE `email` = '$email' AND `ConfirmCode` = '$c_code' AND `ConfirmID` = 0"), 0) == 1)
    {
        mysql_query("UPDATE `users` SET `ConfirmID` = 1 WHERE `email` = '$email'");
        return true;
    } else {
        return false;
    }
...
function change_profile_image($user_id, $file_temp, $file_extn) {
	$file_path = 'user-images/' . substr(md5(time()), 0, 10) . '.' . $file_extn;
...
function user_active($username) {
    $username =sanitize($username);
    return (mysql_result (mysql_query("SELECT COUNT(`UserID`) FROM `users` WHERE `Username` = '$username' AND `ConfirmID` = 1"), 0)  == 1) ?  true: false ;
